...
In a recent project, I was looking to plot data from different variables along the same time axis. The difficulty was, that some of these variables I wanted to have as point plots, while others I wanted to as boxplots.

Because I work with the tidyverse, I wanted to produce these plots with ggplot2. Facetting was the obvious first stept but it took me quite a while to figure out how to best combine facets with point plots (where I have one value per time point) with and boxplots (where I have multiple values per time point).
...
First, for plotting with ggplot2 we want our data in a tidy long format. I also add another column for facetting that groups the variables from `var1` together.
