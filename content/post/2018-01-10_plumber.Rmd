...
We also need to define the input, in our case the test data. When we look at the model object, we see that it expects the following parameters:
...
Good practice is to write the parameter definition into you [API Swagger UI](https://swagger.io/swagger-ui/). We do this by annotating them with name and description in our R-script using `@parameter`. Therefore, I want to know the type and min/max values for each of my variables in the training data. Because the data has been converted to dummy variables if categorical and then scaled and centered, these values will all be numeric and between 0 and 1 in this example. If I would build this script for a real case, I'd use the raw data as input and add a preprocessing function to my script! 
...
To make sure that the model will perform as expected, it is also advised to add a few validation functions. Here, I will validate
...
In this case, we could also consider using `@post` to avoid caching issues, but for now I'll leave it as `@get`.
...
We can also customize the output. By default, the output will be in JSON format. Here, I want to have a text output, so I'll specify `@html`.
...
We can now call our script with the `plumb()` function, run it with `run()` and open it on port 800.
