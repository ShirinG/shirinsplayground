...
Note that I am using the “double-assignment” operator `<<-` in my function, because I want to make sure that objects are overwritten at the top level (i.e. globally). This would have been particularly relevant had I set a global parameter. But just to make sure there is nothing cached from an old session, I am also using it here.
...
- `cat()` outputs to the R console. Using `cat()` is not a good idea, because it's not "serialized". This means that it's not easily understood by clients. 
...
If you wanted to deploy this API, you would need to provide the model and run an R environment with plumber loaded, e.g. on a server. A good way to do this, would be to package everything in a Docker container and run this. Docker will ensure that you have a working snapshot of the system settings, R and package versions that won't change.
