...
Here is an example image from the fruits datasets, which is used in the [practical example for this lesson](https://shirinsplayground.netlify.com/2018/06/keras_fruits/). In general, data can be represented in different formats, e.g. as vectors, tables or matrices. I am using the `imager` package to read the image and have a look at the pixel values. 
...
```{r}
as.array(im)[25:75, 25:75, 1, 1] * 255
```

```{r}
as.array(grayscale(im))[25:75, 25:75, 1, 1] * 255
```

```{r}
channels(im, drop = TRUE)
```

---

Daten können generell in verschiedenen Formaten angeordnet sein, zum Beispiel als Vektor, als Tabelle oder Matrix. Die Pixelwerte für unser Bild sind hier in so einem Matrixformat zu sehen; aus Platzgründen zeige ich aber nur einen Ausschnitt des Bildes, das eigentlich 100 x 100 Pixel groß ist und deshalb Pixelwerte in einer 100 x 100 Matrix hat. Diese Pixelwerte, die wir hier sehen, können entweder die Rohwerte von 0 bis 255 sein oder normierte Werte zeigen; immer stellen sie aber die Helligkeit des Pixels an einer bestimmten Stelle im Bild dar.

Nun ist unsere Erdbeere auf dem Bild aber eigentlich nicht grau sondern rot. Auch Farbbilder können wir mit Pixelmatrizen darstellen. Hier stellen die Werte dann allerdings keine Helligkeiten oder Graustufen dar sondern Farbintensitäten - und zwar meisten für die drei Farbkanäle 
rot,

grün

und blau. Diese Anordnung in drei Dimensionen (also Bildbreite x Bildhöhe x Bildkanäle) wird auch 3-dimensionaler Array genannt.
...
Mit diesen Pixelmatrizen unserer Bilder (dem Input) lernen CNNs nun, die Bilder zu erkennen (zum Beispiel als Klassifikation, d.h. Einordnung in eine Bildkategorie, dem Output). 

Genauer, lernen CNNs 

indem sie verschiedene Abstraktionsebenen der möglichen Klassen finden.

Das heißt, in den ersten versteckten Schichten des neuronalen Netzes werden meistens allgemeine Muster, wie Kanten erkannt.

Je weiter wir in den versteckten Schichten Richtung Output kommen, desto spezifischer werden die erkannten Muster, z.B. Texturen,

grobe Muster,

einzelne Teile der Objekte auf den Bildern

und letztendlich ganze Objekte.

---

Bilder könnten auch mit Multi-Layer Perceptronen (den MLPs) klassifiziert werden, aber in den meisten Fällen sind die nicht besonders gut darin. Also, worin besteht der Unterschied zwischen den klassischen MLPs und CNNs, der CNNs so viel besser darin macht, Bilder zu erkennen?

Der wichtigste Unterschied ist, dass bei MLPs alle Pixel unabhängig voneinander gelernt werden. Das heißt, jede Pixelposition auf dem Bild ist ein unabhängiges Feature. Benachbarte Pixelwerte werden nicht in Betracht gezogen. Deshalb können die Objekte, die auf den Bildern gelernt wurden, nur dann richtig erkannt werden, wenn sie die selbe Anordung auf dem Bild, Orientierung, und so weiter, wie die Trainingsbilder haben. Außerdem wird durch die große Anzahl an Pixeln, also Featuren, die bei Bildern entstehen, die Anzahl der Parameter schnell extrem groß und kann nicht mehr effektiv trainiert werden. Durch Optimierung, z.B. mit Gradient Descent, gelernt werden bei MLPs die Gewichte der Neuronen.
